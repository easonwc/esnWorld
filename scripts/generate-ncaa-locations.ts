...
const NCAA_CAMPUSES: readonly Campus[] = [
  ...
  ["Coral Gables", "Florida", 25.7215, -80.2684, "America/New_York", 49_000],
...
function formatCollegeEntry(entry: CollegeSeedEntry): string {
...
function buildCollegeSeedEntries(): CollegeSeedEntry[] {
  return NCAA_SCHOOLS.map(([name, city, state, attendance]) => ({
    name,
    locationName: city,
...
function main(): void {
  ...
  const locationEntries = buildNcaaLocations(NCAA_CAMPUSES, existingKeys);
  const locationLines = locationEntries.map(formatEntry).join("\n");

  const locationOutput = `import type { LocationSeedEntry } from "./types";
...
`;
  ...
  const locationOutPath = join(
    process.cwd(),
    "src/persistence/seed/ncaa-locations.data.ts",
  );
  writeFileSync(locationOutPath, locationOutput, "utf8");
  console.log(`Wrote ${locationEntries.length} NCAA campus cities to ${locationOutPath}`);

  const collegeEntries = buildCollegeSeedEntries();
  const collegeLines = collegeEntries.map(formatCollegeEntry).join("\n");
  const collegeOutput = `import type { CollegeSeedEntry } from "./types";
...
`;
  ...
  const collegeOutPath = join(process.cwd(), "src/persistence/seed/colleges.data.ts");
  writeFileSync(collegeOutPath, collegeOutput, "utf8");
  console.log(`Wrote ${collegeEntries.length} colleges to ${collegeOutPath}`);
}
