...
import { buildTournamentEventTree } from "@/modules/tennis/materialize";
import {
  orderRotationVenueLinks,
  pickAvailableVenueLink,
  preferredRotationIndex,
} from "@/modules/tennis/venue-pick";
import type { TennisTournament, TennisTournamentVenue } from "@/modules/tennis/types";
...
const rotationTournament: TennisTournament = {
  id: "tournament-1",
  tourId: "tour-1",
  slug: "test-rotation",
  name: "Test Rotation",
...
describe("pickAvailableVenueLink", () => {
  ...
  let venueLinks: TennisTournamentVenue[];
  ...
  beforeEach(async () => {
    ...
    for (const name of ["Host A", "Host B", "Host C", "Host D"]) {
      ...
      for (let index = 0; index < rotationTournament.activeCourtCount; index += 1) {
  ...
  it("walks the rotation pool when the preferred host is already booked", async () => {
    ...
    const courts = await venueResourceRepository.listByVenue(preferredVenueId);
...
describe("orderRotationVenueLinks", () => {
  function venueLink(rotationOrder: number): TennisTournamentVenue {
    return {
      id: `link-${rotationOrder}`,
      tournamentId: rotationTournament.id,
